...
def train_dgl(
    ...
    if config.write_predictions and not classification and config.model.output_features == 1:
        net.eval()
        x = []  
        y = []  
        with torch.no_grad():
            for dat in test_loader:
                ...
                out = net([g.to(device), lg.to(device), lg.to(device)])
             
                x.append(torch.as_tensor(out).cpu().numpy().tolist())     
                y.append(target.cpu().numpy().tolist())               
